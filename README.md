...
- **論理削除**：会員・コンテンツ系テーブル（`users` / `topics` / `comments` / `announcements`）で SoftDeletes を採用。誤操作からのデータ保護に加え、将来的にユーザーやコメントを復元できるようにする意図があり、削除ではなく論理削除を選択（現時点では復元機能自体は未実装。既読管理用の中間テーブル `announcement_reads` や、削除機能自体を持たない `supports` は対象外）
...
- 管理画面から論理削除したユーザー・トピック・コメントの復元機能の実装
